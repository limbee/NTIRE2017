...
cmd:option('-patchSize',    50,                                 'Patch size')
cmd:option('-nSample',      1e6,                                'Number of randomly cropped samples')
cmd:option('-nSave',        1e3,                                'Number of samples to be saved')
...
for i = 1, opt.nSave do
    ...
    if i == 1 or i == opt.nSave then
        print('rank = ' .. i .. ', diff = ' .. diff)
    end
