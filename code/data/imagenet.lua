...
local M = {}
local imagenet = torch.class('sr.imagenet', M)

function imagenet:__init(opt, split)
    self.opt = opt
    self.split = split

    self.size = opt.nTrain_IMAGENET
    ...
    elseif opt.datatype == 't7pack' then
        error('No t7pack support. IMAGENET is too large.')
    else
        error('unknown -datatype (png | t7(default) | t7pack)')
    end

    self.dirTar_IMAGENET = paths.concat(apath, 'IMAGENET_HR')
    self.dirInp_IMAGENET = {}
    for i = 1, #self.scale do
        local orgPath = paths.concat(apath, 'IMAGENET_LR_' .. opt.degrade, 'X' .. self.scale[i])
        local augPath = paths.concat(apath, 'IMAGENET_LR_' .. opt.degrade .. '_augment', 'X' .. self.scale[i])
        
        if opt.augUnkIMAGENET and not paths.dirp(augPath)then
            print(opt.augUnkIMAGENET)
            error('Augmented DIV2K LR unknown dataset does not exist.')
        end

        if not opt.augUnkIMAGENET then
            table.insert(self.dirInp_IMAGENET, orgPath)
        else
            table.insert(self.dirInp_IMAGENET, augPath)
        end
    end
...
function imagenet:get(idx, scaleIdx)
    local idx = idx
    ...
    local function getImg(idx, scale, type)
        ...
        if type == 'DIV2K' then
            ...
            nDigit = 4
        elseif type == 'IMAGENET' then
            dirInp = self.dirInp_IMAGENET[scaleIdx]
            dirTar = self.dirTar_IMAGENET
            nDigit = 5
        end

        if idx > self.opt.imagenetSize then
            idx = idx - self.opt.imagenetSize
    ...
    if self.split == 'val' then
        ...
        if not self.opt.useDIV2K then
            input, target = getImg(idx, scale, 'IMAGENET')
        else
            if idx <= self.opt.imagenetSize then
                input, target, rot = getImg(idx, scale, 'IMAGENET')
...
function imagenet:getFileName(idx, scale, nDigit, type)
    ...
    local rot
    if self.split == 'train' and ((type == 'DIV2K' and self.opt.augUnkDIV2K) or (type == 'IMAGENET' and self.opt.augUnkIMAGENET)) then
        rot = math.random(1,8)
