...
local Adversarial_loss, parent = torch.class('nn.Adversarial_loss','nn.Criterion')

function Adversarial_loss:__init(opt)
    local loaded = false
    if opt.load then
...
function Adversarial_loss:updateOutput(input,target,mode)
...
function Adversarial_loss:updateGradInput(input,target)
    ...
    optim.adam(self.feval, self.params, self.optimState)
